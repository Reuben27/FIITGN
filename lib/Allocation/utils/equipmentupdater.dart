  ...
  for (int i = 0; i < length; i++) {
    if (orders[i] != 0) {
      ...
      if (documentSnapshot.exists) {
        ...
        print(bookedslots);
        numberofbookedslots =
            await documentSnapshot['numberofbookedslots'];
      } else {
        print("no data buoid");
      }
      ...
      int doneornot = await updater(docname, bookedslots, numberofbookedslots, sportequipmentid);
...
Future<int> updater(String docname, var bookedslots, int numberofbookedslots, String sportequipmentid) async {
  CollectionReference equipments = FirebaseFirestore.instance.collection(sportequipmentid);
